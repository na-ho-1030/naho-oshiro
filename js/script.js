...
for (let i = 0; i < animationTargetElements.length; i++) {
  const targetElements = animationTargetElements[i];
  ...
  for (let j = 0; j < texts.split("").length; j++) {
    ...
    if (t === " ") {
     ...
     textsArray.push('<span><span style="animation-delay: ' + ((j * .05) + .3) + 's;">' + t + '</span></span>')
    }
  }

  for (let k = 0; k < textsArray.length; k++) {
    targetElements.innerHTML += textsArray[k]; 
  }
}


const targetElement = document.querySelectorAll(".about-animation,.scroll-in");
